...
func reviewJobTools() []Tool {
	...
	return []Tool{
		...
		{Name: "review_job_list", Title: "Discover implementation/review jobs", Description: "List only the authenticated owner's jobs, newest first, including lifecycle, worker IDs and final evidence availability. Follow next_cursor for another bounded page.", InputSchema: object(map[string]any{"limit": integer(1, 100), "cursor": text("Opaque next_cursor from a previous page")}), Annotations: read, Grant: "review.read", Surfaces: surfaces},
